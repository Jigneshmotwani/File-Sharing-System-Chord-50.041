...
// 		ID:   resp.Successor.Ip,
// 		port: int(resp.Successor.Port),
// 	}

// 	// Get successor's predecessor information
// 	succConn, err := grpc.NewClient(fmt.Sprintf("%s:%d", nextNode.ID, nextNode.port),
...
// 	predConn, err := grpc.NewClient(fmt.Sprintf("%s:%d", prevNode.ID, prevNode.port),
// 		grpc.WithTransportCredentials(insecure.NewCredentials()))

...
func (n *Node) findSuccessor(id []byte) (*RemoteNode, error) {
    ...
    if betweenRightInclusive(n.ID, successor.ID, id, n.KeySize) {
        ...
        client, err := rpc.Dial("tcp", closestNode.Address())
        if err != nil {
            return nil, err
        }
        defer client.Close()
        ...
        err = client.Call("Node.FindSuccessorRPC", args, &reply)
        if err != nil {
            return nil, err
        }
        return reply.Successor, nil
    }
}
...
// FindSuccessorRPC is the RPC method for findSuccessor
func (n *Node) FindSuccessorRPC(args *FindSuccessorArgs, reply *FindSuccessorReply) error {
    successor, err := n.findSuccessor(args.ID)
    if err != nil {
        return err
    }
